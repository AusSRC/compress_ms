...
if(NOT CASACORE_INCLUDE_DIR)
  ...
  # Get casacore's version number
  set(_version_check_src "
#include <iostream>
...
int main(int argc, char *argv[]) {
  ...
  set(_scratch_dir ${CMAKE_CURRENT_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/casacore_version)
  set(_version_check_fname ${_scratch_dir}/casacore_version.cxx)
  file(WRITE ${_version_check_fname} "${_version_check_src}")
  file(APPEND ${_version_check_fname} "\n")
  try_compile(_success ${_scratch_dir} ${_version_check_fname}
              CMAKE_FLAGS "-DINCLUDE_DIRECTORIES=${CASACORE_INCLUDE_DIR} "
              LINK_LIBRARIES ${CASA_CASA_LIBRARY}
              COPY_FILE ${_scratch_dir}/casacore_version)
  if (NOT _success)
    message(FATAL_ERROR "Failed to compile casacore version dumping program")
  endif()
  execute_process(
      COMMAND ${_scratch_dir}/casacore_version
      OUTPUT_VARIABLE CASACORE_VERSION)
  if (NOT CASACORE_VERSION)
    message(FATAL_ERROR "Didn't get any version out of casacore")
  endif()
  # There have been hiccups historically in the version casacore reports
  # We don't automatically adjust; instead we let users know and let them deal with it
  if (CASACORE_VERSION STREQUAL trunk)
    set(_extra_info " (i.e., <2.3.0)")
  elseif (CASACORE_VERSION STREQUAL 2.5.0)
    set(_extra_info " (i.e., ==3.0.0)")
  endif()
  message(STATUS "Found casacore version ${CASACORE_VERSION}${_extra_info}")
  mark_as_advanced(CASACORE_VERSION)
  # Easy-to-inspect variable for casacore3+ support
  if (NOT CASACORE_VERSION STREQUAL trunk AND
      CASACORE_VERSION VERSION_GREATER 2.4.0)
    set(CASACORE3 ON)
  endif()
  mark_as_advanced(CASACORE3)
endif(NOT CASACORE_INCLUDE_DIR)
...
# Set Casacore_FOUND for find_package/find_dependency compatibility
set(Casacore_FOUND ${CASACORE_FOUND})
